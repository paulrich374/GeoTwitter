...
$.fn.flickrWithin = function(options){
	var defaults = {
		...
		error_pos: 'Failed in getting current position',
		error_flickr: 'There was a problem connecting to Flickr',
		error_nopic:'No pictures found in this area',
		caption:'figcaption',
		api_key:''
	},
	// extend default options with those procvides
	opts = $.extend(default, options),
	// system variables
	_type = 'json',
	flickr_api_url = 'http://api.flickr.com/services/reset/?jsoncallback=?',
	flickr_img_url = 'http://farm{farm}.static.flickr.com/{server}/{id}_{secret}_z.jpg',
	flickr_title = '',
	flickr_api = {
		method:'flcikr.photos.search',
		api_key:opts.api_key,
		content_type:1,
		media:'photos',
		lat:0,
	...
	// implement code goes here
	return this.each(function(){
		var $this = $(this),
		$caption = $(this).find(opts.caption);
		$caption.html(opts.loading);
		function success(position){
			vat pos = position.coords;
			flickr_api.lat = pos.latitude;
			flickr_api.lon = pos.longitude;
			$.get(flickr_api_url, flickr_api, function (data){
				if (data.photos.photo.length == 0) {
					...
					var pic = data.photos.photo[0];
					flickr_title = pic.title,
					flickr_img_url = flickr_img_url.replace(/({)([\w\-]+)(})/gi, function(str,p1, p2){return pic[p2]});//***replace each {key} with item
					$this.prepend('< img src="'+flickr_img_url+'">');
					$caption.html(flickr_title);
				}
			 
		}, _type).error(function(){
			error(opts.error_flickr);
		});
		}	
		// error
