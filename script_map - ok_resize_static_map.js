        ...
        init: function(){
            ...
            */
			...
			google.maps.event.addListener(map, 'click', function(event) {
    			...
    			//alert(myLatLng);
				//$('#map_canvas').css({ width: "100%", height: "50%" });
...
var center = myLatLng;//map.getCenter();
//alert(center);
...
map.setCenter(center); 				
		...
		/*
				var southwest = new google.maps.LatLng(lat - dw,lng + dh);
				var northeast = new google.maps.LatLng(lat + dw,lng - dh);
...
//alert("44444");		
		var zf = Math.pow(2, minZoomLevel);
		var dw = 0.02;//$( window ).height() / zf;
		var dh = 0;//$( window ).width()/ zf;
		//alert(zf+",  "+dw+", "+dh);				
				window.setTimeout(function() {
...
function loadNearbyTweet(lat, lng){
    ...
    sizeString = 'size=600x400';
    typeString = '&maptype=' + map.mapTypeId;
    /*
	if(markersArray.length != 0){
        for(i=0 ; i < markersArray.length ; i++){
            markerString = markerString + markersArray[i].getPosition().toUrlValue() + '|';
        };
    }
	*/ 
	/*
	if(pushPin.length != 0){
        for(i=0 ; i < pushPin.length ; i++){
        	//alert(pushPin[i]);
            markerString = markerString + pushPin[i].position().toUrlValue() + '|';
        };
    }
    */	

    // OK mapString = defaultMapString + centerString +  zoomString + typeString  + '&' + markerString + '&size=600x200'+'&sensor=false';

	
	/* Info Window - Header */
	var headerHTML = '';
	var loadingHTML = '';
	//headerHTML += '<img src="'+mapString+'" style="float:left;padding:3px 12px 0px 6px" alt="" />';
